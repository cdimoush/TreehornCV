...
class App:
    def __init__(self, video_src):
        ...
        self.y_displacement_threshold = 25  # Threshold for significant y-displacement
    ...
    def run(self):
        while True:
            ...
            if len(self.tracks) > 0:
                ...
                for tr, (x, y), good_flag in zip(self.tracks, p1.reshape(-1, 2), good):
                    ...
                    if len(tr) > self.track_len:
                        del tr[1]  # Keep the trajectory length under `track_len`
                    new_tracks.append(tr)
                    # Determine the color based on y-displacement
                    disp = self.calculate_y_displacement(tr)
                    if abs(disp) > self.y_displacement_threshold:
                        if disp > 0: # Moving down (Red)    
                            color = (0, 0, 255)
                        else: # Moving up (Blue)
                            color = (255, 0, 0)
                    else: # Not moving (Green)
                        color = (0, 255, 0)
                    # Draw a small circle at the new point position
                    cv.circle(vis, (int(x), int(y)), 2, color, -1)
                    # Display the ID next to the circle
                    cv.putText(vis, str(tr[0][0]), (int(x) + 5, int(y) - 5),
                               cv.FONT_HERSHEY_SIMPLEX, 0.4, color, 1)
                self.tracks = new_tracks
                # Draw the trajectories
                for tr in self.tracks:
                    cv.polylines(vis, [np.int32([pt[1] for pt in tr])], False, (0, 255, 0))
            ...
            if self.frame_idx % self.detect_interval == 0:
                ...
                mask[:] = 255
                for x, y in [np.int32(tr[-1][1]) for tr in self.tracks]:
                ...
                if p is not None:
                    ...
                    for x, y in np.float32(p).reshape(-1, 2):
                        ...
                        self.next_id += 1  # Increment the ID for the next new point
    ...
    def calculate_y_displacement(self, track):
        # Calculate the total y-displacement for a given track using the trail
        return sum(track[i][1][1] - track[i-1][1][1] for i in range(2, len(track)))
