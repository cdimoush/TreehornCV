...
class WaveGUI:
    ...
    def update_plot(self, factor=100):
        ...
        # Highlight portions of the plot where the up key is pressed
        up_highlight_theta = [np.pi/2 if state else -np.pi/2 for state in self.key_state_vals]
        self.up_key_highlight_line.set_data(self.t_vals, up_highlight_theta)

        # Highlight portions of the plot where the down key is pressed
        down_highlight_theta = [-np.pi/2 if state else np.pi/2 for state in self.key_state_vals]
        self.down_key_highlight_line.set_data(self.t_vals, down_highlight_theta)
